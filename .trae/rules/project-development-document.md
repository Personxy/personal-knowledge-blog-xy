...
### 8.1 API 文档维护
- **原则**: 代码变更必须同步更新 API 文档。
- **执行**: 每次新增、修改或删除 API 接口代码（包括路由、参数、返回值变更）后，必须立即更新根目录下的 `API_DOCUMENTATION.md` 文件。
- **内容**: 确保文档中的 URL、请求方法、鉴权要求、请求参数（Query/Body）、返回结构（成功/失败示例）与实际代码保持严格一致。
